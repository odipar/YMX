...
| `ymxsndh-k1-copies-v<release>.bin`, `-k2-copies`, `-k4-copies` | the same with the copy code built in: the core a tune with copies from the literal stream needs (`SPEC.md` §1.2, flag bit 5). It reads the window off each tune's ring at init, so it serves any ring |
...
sizes by the four flag combinations, with and without the copy code - and
the stub; one `ym-to-ymx` zip per
...
| 20 | 2 | flags: bit 0 = raster monitor built in, bit 1 = frame write unmasked, bit 2 = the copy code is built in |
...
2. **Pick the core** for the unit size the tunes are packed at - the
   ...
   has flag bit 5 set; that core reads the window off each tune's ring at
   init, so one serves any ring. Verify its descriptor (§1): `'YMXC'`,
   descriptor version 2, the tunes' format version, the unit and the
   flags.
