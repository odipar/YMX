...
multiplies. A permanent tax for bytes that were never the pressure point —
the pressure points are cycles and RAM, and clustering moves neither.

**The door left open:** if file size ever becomes the constraint, the cheap
...
half-parity was random per re-start.
...
maxYMiser's, and `RELEASE` grew a mask flag for it. The packer chooses per
tune with `-sidresume`. Since the choice is per emitted byte, a packer could
switch models mid-song; nothing in the format forbids it.
...
3. **Distrust the capture before the player.** A glitched recording of correct
   ...
   is validated.
4. **The ear is an instrument.** It found what the audit called perfect, twice
   in two days.
...
1. **"Identical chip writes" is not "identical playback."** The bus is not the
   whole machine.
...
**The unmasked burst** (2026-08-21). The frame write masked interrupts, and
the comment counting the two `sr` instructions called that cheap. It counted
the wrong thing: the cost was the ~500 cycles of interrupt latency they
guarded, paid by whichever tick fell inside — longer than a whole tick period
at the top of the timer range.
...
different from the game, asked properly.

- **Melody: exact.** Every tone period matches the original replay — 0.00
  semitones of offset. Nobody transposed anything.
...
faithful Chambers of Shaolin is the SNDH; the dump's drums are a cover
version.

The hunt did expose one real weakness on the way: the drum rescue used to
